...
export default function CategoriesList() {
    ...
    if (status === "loading") {
        return <p className="p-10 text-center text-amber-950 dark:text-amber-200">Cargando categorias...</p>;
    }

    if (status === "error") {
        return <p className="p-10 text-center text-amber-950 dark:text-amber-200">Error al cargar las categorias.</p>;
    }
    ...
    return (
        <main className="min-h-full bg-gray-200 text-amber-950 dark:bg-slate-900 dark:text-slate-100 mt-4">
            <Header />
            <section className="mt-6 mx-auto flex w-full max-w-5xl flex-col gap-4 rounded-2xl border-gray-200 bg-gray-200 dark:border-slate-950 dark:bg-slate-950/40 p-4 sm:p-6">
                <header className="flex items-center justify-between border-b border-amber-950 dark:border-slate-800 pb-4">
                    <h2 className="text-2xl font-semibold tracking-tight text-amber-950 dark:text-yellow-100">Categorías</h2>
                    <Link 
                        to="/categorias/new"
                    ...
                    {categoriasFiltradas && categoriasFiltradas.length > 0 ? (
                        categoriasFiltradas.map((categoria) => (
                            <article
                                ...
                                className="flex items-center justify-between gap-4 rounded-xl border border-amber-950 bg-amber-100 dark:border-slate-800 dark:bg-slate-800/80 px-4 py-4"
                            >
                        ...
                        <p className="py-6 text-center text-amber-950 dark:text-amber-200">
